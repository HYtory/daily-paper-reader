...
tldr: 安全强化学习需要在复杂环境中避免危险行为，但现有方法在约束下学习缓慢且任务性能次优。本文提出ATACOM方向约束（ATACOM-DC），在ATACOM安全层基础上区分动作是接近还是远离约束边界，仅在必要时激活约束。在多种仿真机器人控制任务中，该方法显著降低了违规成本并保持了任务性能，为安全RL提供了更高效的探索策略。
source: arxiv
...
motivation: 现有安全强化学习方法在约束下学习速度慢，任务性能次优。
method: 提出ATACOM-DC，在ATACOM框架上引入方向约束，仅约束朝向边界的动作。
result: 在仿真机器人任务中，显著降低违规成本且保持甚至提升任务性能。
conclusion: 方向约束有效改善了安全RL的探索效率与性能权衡。
---

## 摘要
强化学习彻底改变了机器人研究的格局，使得在仿真中能够稳健地学习复杂的机器人技能。然而，在开放环境中进行实际部署需要强大的安全保证，以防止危险或有害行为。安全强化学习方法通过强制执行安全约束来满足这一要求。然而，在约束条件下学习通常会降低学习速度，并可能导致次优的任务性能，因为与无约束设置相比，代理必须解决更复杂的约束优化问题。为了解决这个问题，在这项工作中，我们提出了ATACOM框架的一个扩展，这是一种最先进的可信安全层，可以与现有的强化学习算法集成，以强制执行从系统先验知识或直接从数据中学习到的约束。我们提出的方法命名为ATACOM方向性约束（ATACOM-DC），通过引入方向性约束来显著改善安全-性能权衡，这些约束区分接近和远离约束边界的动作，仅在必要时激活约束强制执行。我们在仿真中的一系列具有挑战性的机器人控制任务上评估了我们的方法，分析了约束违反成本和实现的任务性能。代码和更多材料见 https://atacom-dc.robot-learning.net。
