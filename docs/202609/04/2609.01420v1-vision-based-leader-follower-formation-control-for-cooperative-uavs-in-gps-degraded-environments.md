...
tldr: 在GPS退化环境下，多无人机协同需要可靠的相对感知。本文提出一种基于视觉的领航-跟随编队框架，跟随无人机使用前视RGB-D相机，通过轻量YOLO检测器识别领航无人机，并结合深度信息估计相对位姿。该框架在XTDrone物理仿真中验证，在定位传感器故障下仍能保持稳定编队，计算开销合理，为真实系统提供基础。
source: arxiv
...
motivation: GPS退化时需视觉替代方案实现多无人机编队保持。
method: 基于YOLO检测与RGB-D深度估计领航无人机相对位姿。
result: 仿真中检测准确且编队误差稳定，计算成本低。
conclusion: 为视觉协同编队控制提供实用基础。
---

## 摘要
多无人机系统中的协同要求可靠的相对感知，使得即使绝对定位传感器退化或失效，跟随飞行器也能保持编队并安全地继续执行任务。本文提出了一种在跟随无人机上运行的基于视觉的协同编队框架，该框架使用前向RGB-D相机实时检测、跟踪和定位领航无人机。轻量级YOLO检测器在专用无人机数据集上训练并部署于机载，用于预测领航者边界框，然后通过针孔相机模型与深度信息融合，估计领航者的相对姿态。这些估计为领航-跟随位置控制器提供支持，并且可在GPS或外部定位不可用时作为备份。该框架实现为一组ROS节点，并在基于XTDrone构建的物理多无人机仿真中，考虑了传感器噪声和通信中断，进行评估。我们评估了在标称条件以及模拟定位传感器故障下的检测精度、运行时间和编队保持误差。结果表明，所提框架以合理的计算成本维持稳定的领航-跟随编队，并为将基于视觉的协同编队控制扩展到实际多无人机系统提供了实用基础。
